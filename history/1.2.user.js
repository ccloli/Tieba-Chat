		...
		if(xhr.readyState==4&&xhr.status==200){
			...
			if(t.error_code=='0'){
				...
				if(!document.getElementsByClassName('tb_chat_msg_panel')[0]){
					...
					window.onfocus=function(){if(this_msg_update==null)this_msg_update=setInterval(function(){if(document.getElementsByClassName('tb_chat_msg_panel')[0])get_latest_msg(id)},10000);};
...
var message_update_timer=setInterval(function(){
	if((new Date().getTime()-(window.localStorage.getItem('message_update_timestamp')||message_update_timestamp))>10000){
		get_message_update();
	}
...
panel_top.onmouseover=function(){
	panel.setAttribute('show','1');
	if(this_username==null)get_chatting_list(chatting_list_page);
};
