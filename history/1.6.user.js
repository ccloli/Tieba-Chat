	...
	xhr.onreadystatechange=function(){
		if(xhr.readyState==4&&xhr.status==200){
			...
			if(t.error_code=='0'){
				...
				//if(panel_top_left.hasAttribute('show'))panel_top_left.removeAttribute('show');
				//if(panel_top_left.hasAttribute('show')){
					//panel_top_right.removeAttribute('show');
		...
		if(xhr.readyState==4&&xhr.status==200){
			...
			if(t.error_code=='0'){
				...
				if(silent==false){
					...
					//panel_top_left.setAttribute('show','1');
					//panel_top_right.setAttribute('show','1');
...
function insert_chat_button(event){
	if(event.animationName=='nodeInserted'){
		...
		var username=JSON.parse(node.parentElement.parentElement.parentElement.parentElement.getAttribute('data-field')).un;
		button.className='btn_chat';
		node.parentElement.insertBefore(button,node);
		button.onclick=function(){get_userid(username);panel.setAttribute('show','1');};
	}
}
