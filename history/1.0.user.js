...
var user_id=unsafeWindow.PageData.user.user_id||unsafeWindow.PageData.user.id;//,
	//chatting_list_data='';
...
if(document.cookie.match(/BDUSS=(.{192,}?);?/))BDUSS=document.cookie.match(/BDUSS=(.{192,}?);?/)[1];
else{
...
var tb_chat_data=JSON.parse(unsafeWindow.localStorage.getItem('tb_chat_data')/*||unsafeWindow.localStorage.getItem('tb_chat_data')*/)||{};
var message_update_timestamp=unsafeWindow.localStorage.getItem('tb_chat_message_update_timestamp')||new Date().getTime();
var message_update_count=unsafeWindow.localStorage.getItem('tb_chat_message_update_count')||0;
var user_portrait='';
var com_portrait='';
	...
	xhr.onreadystatechange=function(){
		if(xhr.readyState==4&&xhr.status==200){
			...
			if(t.error_code=='0'){
				...
				unsafeWindow.localStorage.setItem('tb_chat_message_update_timestamp',message_update_timestamp);
				unsafeWindow.localStorage.setItem('tb_chat_message_update_count',t.message.pletter);
			}
			else{
	...
	xhr.onreadystatechange=function(){
		if(xhr.readyState==4&&xhr.status==200){
			...
			if(t.error_code=='0'){
				...
				if(id!=this_userid||document.getElementsByClassName('tb_chat_panel_list')[0]){
					panel_main.innerHTML='';
					var panel_msg=document.createElement('div');
					panel_msg.className='tb_chat_panel_msg';
					panel_main.appendChild(panel_msg);
				}
				else var panel_msg=document.getElementsByClassName('tb_chat_panel_msg')[0];
				...
				if(!document.getElementsByClassName('tb_chat_msg_panel')[0]){
					...
					unsafeWindow.onfocus=function(){if(this_msg_update==null)this_msg_update=setInterval(get_latest_msg(id),10000)};
					unsafeWindow.onblur=function(){if(this_msg_update!=null){clearInterval(this_msg_update);this_msg_update=null}};
...
function add_enterance(){
	...
	for(var i=0;i<s.length;i++){
		if(s[i].hasAttribute('tb_chat_added')==false){
			...
			j.textContent='聊天';
			s[i].appendChild(j);
			j.onclick=function(){var ud=JSON.parse(this.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.getAttribute('data-field')).author;this_userid=ud.user_id;this_username=ud.user_name;panel.setAttribute('show','1');get_latest_msg(ud.user_id)}
			var k=document.createTextNode(' |');
			s[i].appendChild(k);
...
var message_update_timer=setInterval(function(){
	if((new Date().getTime()-(unsafeWindow.localStorage.getItem('message_update_timestamp')||message_update_timestamp))>10000){
		get_message_update();
	};
	if(message_update_count!=unsafeWindow.localStorage.getItem('tb_chat_message_update_count')){
		message_update_count=unsafeWindow.localStorage.getItem('tb_chat_message_update_count');
		if(message_update_count!='0'){
			document.title='【'+message_update_count+' 条新聊天】'+original_title;
			panel_top.style.color='orange';
		}
...
panel_top_back.onclick=function(){get_chatting_list(chatting_list_page)};
panel_top.onmouseover=function(){panel.setAttribute('show','1');if(this_username==null)get_chatting_list(chatting_list_page)};
panel.onmouseover=function(){if(panel_ifshow!=null)clearTimeout(panel_ifshow)}
panel.onmouseout=function(){panel_ifshow=setTimeout(function(){if(panel.hasAttribute('show'))panel.removeAttribute('show');panel_ifshow=null},500)};
